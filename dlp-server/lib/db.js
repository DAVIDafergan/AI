...
const TenantSchema = new mongoose.Schema(
  {
    ...
    remoteInstall: {
      ...
      sshAuthMethod: { type: String, enum: ["password", "privateKey"], default: "password" },
      sshPassword:   { type: String, select: false },
      sshPrivateKey: { type: String, select: false },
      installDir:    { type: String, default: "/opt/ghostlayer" },
      status:        { type: String, enum: ["idle", "installing", "online", "offline", "error"], default: "idle" },
      agentUrl:      { type: String, default: "" },
...
const TenantEventSchema = new mongoose.Schema({
  ...
  eventType: {
    type: String,
    enum: ["scan", "block", "alert", "agent_connect", "agent_disconnect", "config_change", "user_action", "agent_provision_error"],
    required: true,
  },
