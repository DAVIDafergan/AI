...
- Node.js 18+
- npm 9+
- **MongoDB 6+** – נדרש לפיצ'רים של Multi-tenancy (טבלת לקוחות, סוכנים, יומן אירועים)
  - בסביבת פיתוח ניתן להשתמש ב-[MongoDB Atlas](https://www.mongodb.com/atlas) (חינמי) או להריץ `docker run -d -p 27017:27017 mongo:7`
  - הגדר `MONGODB_URI` ב-`.env.local` (ראה הגדרת סביבה למטה)
...
| `MONGODB_URI` | - | **חובה לפיצ'רי Multi-tenancy** – למשל `mongodb://localhost:27017/ghostlayer` |
| `SUPER_ADMIN_KEY` | - | מפתח גישה ל-Super Admin Dashboard (הגדר ערך חזק) |
